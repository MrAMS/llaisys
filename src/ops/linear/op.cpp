...
void linear(tensor_t out, tensor_t in, tensor_t weight, tensor_t bias) {
    ...
    if(!bias->isNone()) CHECK_SAME_DEVICE(out, bias);
    ...
    if(!bias->isNone()) CHECK_ARGUMENT(bias->ndim()==1, "Linear: bias must be a 1D tensor");
    ...
    if(!bias->isNone()) CHECK_ARGUMENT(out->shape()[1] == bias->shape()[0], "Linear: out tensor shape must match bias tensor");
    ...
    if(!bias->isNone()) CHECK_ARGUMENT(bias->isContiguous(), "Linear: bias must be a contiguous tensor");
