...
void linear(tensor_t out, tensor_t in, tensor_t weight, tensor_t bias) {
    ...
    if(bias->ndim()!=0) CHECK_SAME_DEVICE(out, bias);
    CHECK_SAME_DTYPE(out->dtype(), in->dtype(), weight->dtype());
    if(bias->ndim()!=0) CHECK_SAME_DTYPE(out->dtype(), bias->dtype());
    ...
    if(bias->ndim()!=0) CHECK_ARGUMENT(bias->ndim()==1, "Linear: out must be a 1D tensor");
    ...
    if(bias->ndim()!=0) CHECK_ARGUMENT(out->shape()[1] == bias->shape()[0], "Linear: out tensor shape must match bias tensor");
    ...
    if(bias->ndim()!=0) CHECK_ARGUMENT(bias->isContiguous(), "Linear: bias must be a contiguous tensor");
