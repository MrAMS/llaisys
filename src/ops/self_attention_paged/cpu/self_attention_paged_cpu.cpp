#include "self_attention_paged_cpu.hpp"
...
template<typename T>
void self_attention_paged_(T *attn_val, const T *q, const T* const* k_map, const T* const* v_map, 
            ...
            for(size_t h=0;h<d_head;++h){
                ...
                for(size_t j=0;j<d_tot;++j){ // 遍历scores矩阵的每一列
                    ...
                    for(size_t l=0;l<d_qk;++l){ // 计算Q@K.T
                        ...
                        // [j, h_kv, l]
                        const auto k_idx = h_kv*d_qk + l; 
                        attn_scores[j] += TOF(q[q_idx]) * TOF(k_map[j][k_idx]);
                    }
                    attn_scores[j] *= scale;
                }
                // 计算Causal Softmax
                ...
                for(size_t j=0;j<d_v;++j){ // 遍历out的每一列
                    ...
                    for(size_t l=0;l<causal_len;++l){ // 计算scores @ V
                        ...
                        // [l, h_kv, j]
                        const auto v_idx = h_kv*d_v + j;
                        const auto a_idx = l;
                        res += attn_scores[a_idx] * TOF(v_map[l][v_idx]);
                    }
...
void self_attention_paged(std::byte *attn_val, const std::byte* q, const std::byte* const* k_map, const std::byte* const* v_map, 
    ...
    llaisysDataType_t type){
        ...
        case LLAISYS_DTYPE_F32:
            return self_attention_paged_(reinterpret_cast<float_t *>(attn_val),
                reinterpret_cast<const float_t *>(q),
                reinterpret_cast<const float_t * const*>(k_map),
                reinterpret_cast<const float_t * const*>(v_map),
                scale, d_seq, d_head, d_v, d_qk, d_kvhead, d_tot);
        case LLAISYS_DTYPE_BF16:
            return self_attention_paged_(reinterpret_cast<bf16_t *>(attn_val),
                reinterpret_cast<const bf16_t *>(q),
                reinterpret_cast<const bf16_t * const*>(k_map),
                reinterpret_cast<const bf16_t * const*>(v_map),
                scale, d_seq, d_head, d_v, d_qk, d_kvhead, d_tot);
        case LLAISYS_DTYPE_F16:
            return self_attention_paged_(reinterpret_cast<fp16_t *>(attn_val),
                reinterpret_cast<const fp16_t *>(q),
                reinterpret_cast<const fp16_t * const*>(k_map),
                reinterpret_cast<const fp16_t * const*>(v_map),
                scale, d_seq, d_head, d_v, d_qk, d_kvhead, d_tot);
